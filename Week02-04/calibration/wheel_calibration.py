...
def calibrateWheelRadius():
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        # For 1 meter: scale = distance / (wheel_vel * delta_time)
        # distance = 1.0 meter
...
def calibrateBaseline(scale):
    ...
    baseline = 0
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        # TODO: replace with your code to compute the baseline parameter using wheel_vel, scale, and delta_time
        # For a 360 degree (2*pi radians) spin:
        # baseline = (wheel_vel * scale * delta_time) / pi
