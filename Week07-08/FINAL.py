...
class AutoOperateDynamic(Operate):
    ...
    # Post-hold reverse behavior
        self.fruit_post_hold_reverse_time = 0.5  # seconds to reverse after holding at a fruit
    ...
    def auto_nav_step(self):
        ...
        try:
            now_cov = time.time()
            # Do not start or run covariance spin during fruit hold, any emergency phase, or arrival (waypoint) spin
            if self._mode != 'fruit_hold' and getattr(self, '_emergency_mode', None) is None and self._arrival_spin_start is None:
                ...
                # While holding at fruit OR in emergency OR arrival spin: cancel any ongoing covariance spin and defer
