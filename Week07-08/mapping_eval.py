...
if __name__ == '__main__':
    ...
    if slam_only:
        ...
        for object_err in object_est_errors_aligned:
            ...
            if est_err > max_error:
                target_score = 0
            elif est_err <= min_error:
                target_score = 1
            else:
                target_score = float((max_error-est_err)/(max_error-min_error)) 
            err_lst.append(est_err)
            target_scores.append(target_score)
        
        base = 8
        target_pose_estimation_mark = (base**np.mean(target_scores) - 1)/(base - 1)
