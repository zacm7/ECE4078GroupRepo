...
def main():
    ...
    # debug files
    week0708_dir = os.path.join(REPO_ROOT, 'Week07-08')
    debug_log_path = os.path.join(week0708_dir, 'lab_output', 'mapless_debug.log')
    try:
        debug_f = open(debug_log_path, 'a')
    except Exception:
        debug_f = None
    ...
    while running:
        ...
        # 1) Run navigation logic (this sets operate.command['motion'])
        operate.auto_nav_step()

        # 2) Immediately print the high-level command the nav logic produced
        try:
            high_cmd = operate.command.get('motion')
            line = f"{time.time():.3f} [HIGH CMD] {high_cmd}"
            print(line)
            if debug_f:
                debug_f.write(line + "\n")
        except Exception:
        ...
        # 3) Snapshot EKF pose before control
        try:
            pre_pose = operate.get_pose()
            line = f"{time.time():.3f} [EKF PRE] pose={pre_pose}"
            print(line)
            if debug_f:
                debug_f.write(line + "\n")
        except Exception:
        ...
        # 4) Call control() (this returns the drive_meas object the system will use / send)
        try:
            drive_meas = operate.control()
        except Exception as e:
            drive_meas = None
            line = f"{time.time():.3f} [CONTROL ERR] {e}"
            print(line)
            if debug_f:
                debug_f.write(line + "\n")

        # 5) Introspect and pretty-print the drive_meas object (important!)
        try:
            if drive_meas is None:
                line = f"{time.time():.3f} [DRIVE OUT] None"
                print(line)
                if debug_f:
                    debug_f.write(line + "\n")
            else:
                # Try several ways to show internal numeric values
                printed = None
                try:
                    d = None
                    try:
                        d = dict(vars(drive_meas))
                    except Exception:
                        try:
                            d = dict(drive_meas.__dict__)
                        except Exception:
                            d = None
                    if not d:
                        # Fallback: enumerate attributes that are not callable/private
                        d = {}
                        for a in dir(drive_meas):
                            if a.startswith('_'):
                                continue
                            try:
                                val = getattr(drive_meas, a)
                                if callable(val):
                                    continue
                                # skip large or non-primitive objects
                                if isinstance(val, (int, float, str, bool, list, tuple, dict)):
                                    d[a] = val
                            except Exception:
                                continue
                    printed = pprint.pformat(d)
                    line = f"{time.time():.3f} [DRIVE OUT] type={type(drive_meas)} attrs={printed}"
                    print(line)
                    if debug_f:
                        debug_f.write(line + "\n")
                except Exception as e:
                    line = f"{time.time():.3f} [DRIVE OUT] <unprintable> {e}"
                    print(line)
                    if debug_f:
                        debug_f.write(line + "\n")
        except Exception:
        ...
        # 6) Update SLAM with the drive_meas and log EKF after
        try:
            operate.update_slam(drive_meas)
            post_pose = operate.get_pose()
            line = f"{time.time():.3f} [EKF POST] pose={post_pose}"
            print(line)
            if debug_f:
                debug_f.write(line + "\n")
        except Exception as e:
            line = f"{time.time():.3f} [EKF ERR] {e}"
            print(line)
            if debug_f:
                debug_f.write(line + "\n")
        ...
        # 8) force periodic JSON log flush so mapless_log.json appears
        try:
            nowt = time.time()
            if (nowt - last_forced_flush) > 2.0:
                try:
                    operate._flush_log(force=True)
                    last_forced_flush = nowt
                    line = f"{time.time():.3f} [FLUSH] wrote {operate._log_path}"
                    print(line)
                    if debug_f:
                        debug_f.write(line + "\n")
                except Exception as e:
                    line = f"{time.time():.3f} [FLUSH ERR] {e}"
                    print(line)
                    if debug_f:
                        debug_f.write(line + "\n")
        except Exception:
