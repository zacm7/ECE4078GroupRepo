...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--inflate_aruco_points", type=int, default=8,
                        help="Number of points around each ArUco to emulate a circular obstacle")
...
def main() -> None:
    ...
    # Inflate ArUco markers into obstacle points so planner keeps clear
    try:
        if getattr(args, 'inflate_aruco', False):
            inflate_r = float(getattr(args, 'inflate_aruco_radius', 0.12))
            inflate_pts = int(getattr(args, 'inflate_aruco_points', 12))
        else:
            inflate_r = 0.0
            inflate_pts = 0

        if aruco_pos is not None and len(aruco_pos):
            # Ensure lists exist
            if getattr(operate, 'known_obstacles', None) is None:
                operate.known_obstacles = []
            if getattr(operate, 'discovered_obstacles', None) is None:
                operate.discovered_obstacles = []

            for idx, p in enumerate(aruco_pos):
                try:
                    ax, ay = float(p[0]), float(p[1])
                except Exception:
                    continue

                # Add central marker point if not present
                if not any(abs(ax - ox) < 1e-6 and abs(ay - oy) < 1e-6 for ox, oy in operate.known_obstacles):
                    operate.known_obstacles.append([ax, ay])

                # Add ring of points around the marker to provide clearance
                if inflate_r and inflate_r > 0.0:
                    for k in range(inflate_pts):
                        theta = 2.0 * math.pi * float(k) / float(inflate_pts)
                        px = ax + inflate_r * math.cos(theta)
                        py = ay + inflate_r * math.sin(theta)
                        if not any(math.hypot(px - ox, py - oy) < 1e-6 for ox, oy in operate.known_obstacles):
                            operate.known_obstacles.append([px, py])

                # Add discovered_obstacles entry for GUI/logging if missing
                duplicate = False
                for d in operate.discovered_obstacles:
                    if abs(d.get('x', 0.0) - ax) < 1e-6 and abs(d.get('y', 0.0) - ay) < 1e-6:
                        duplicate = True
                        break
                if not duplicate:
                    operate.discovered_obstacles.append({'x': ax, 'y': ay, 'label': f'aruco{idx}', 'count': 10})
    except Exception:
        pass
