...
class AutoOperateDynamic(Operate):
    """RRT to fixed targets with online obstacle discovery + robust hand-off logic + 30cm merging."""
    ...
    def __init__(self, args, search_list: List[str], targets_xy: List[List[float]],
        ...
        self.reached_time: Optional[float] = None
        self.active = True

        # Tolerances/commands
        self.waypoint_tol = 0.04       # 4 cm to accept intermediate waypoints
        self.final_target_tol = 0.15   # 20 cm to accept final target
        ...
        # Detection handling
        self.add_cooldown = 0.5  # seconds between adds
        # --- 30 cm merging settings ---
        self.merge_radius = 2
        self.merge_cooldown = 0.5
        self._last_merge_time = 0.0
        # Use same radius for duplicate suppression
        self.min_obs_separation = self.merge_radius
    ...
    def _rrt_bounds_and_obstacles(self):
        ...
        default_r = 0.15 # typical ArUco/fruit safety radius
        obstacles = []
    ...
    # ======== 30 cm merge helper ========
    def _merge_discovered_obstacles(self):
        """Greedy cluster merge of discovered obstacles within self.merge_radius.
           If a merged center lies within merge_radius of any known obstacle,
           drop it (keep the known one as the anchor)."""
        if not self.discovered_obstacles:
            return

        pts = [np.array(p, dtype=float) for p in self.discovered_obstacles]
        clusters = []  # each: {'sum': np.array([x,y]), 'n': int, 'center': np.array([x,y])}

        for p in pts:
            placed = False
            for c in clusters:
                if np.linalg.norm(p - c['center']) <= self.merge_radius:
                    c['sum'] += p
                    c['n'] += 1
                    c['center'] = c['sum'] / c['n']
                    placed = True
                    break
            if not placed:
                clusters.append({'sum': p.copy(), 'n': 1, 'center': p.copy()})

        merged_centers = [c['center'] for c in clusters]

        # Remove any merged center that sits close to a known obstacle
        kept = []
        for c in merged_centers:
            too_close_to_known = any(
                np.linalg.norm(c - np.array(k, dtype=float)) <= self.merge_radius
                for k in self.known_obstacles
            )
            if not too_close_to_known:
                kept.append([float(c[0]), float(c[1])])

        changed = (len(kept) != len(self.discovered_obstacles)) or any(
            np.linalg.norm(np.array(a) - np.array(b)) > 1e-6
            for a, b in zip(sorted(self.discovered_obstacles), sorted(kept))
        )
        self.discovered_obstacles = kept
        return changed

    # ============= Perception integration =============
    def periodic_perception_update(self):
        """Add unknown obstacles from detector; merge within 30cm; replan if needed."""
        bboxes = getattr(self, 'detector_output', None)
        if not isinstance(bboxes, (list, tuple)) or len(bboxes) == 0:
            # Even if nothing new, occasionally run merge to tidy clusters
            now2 = time.time()
            if (now2 - self._last_merge_time) >= self.merge_cooldown:
                changed = self._merge_discovered_obstacles()
                self._last_merge_time = now2
                if changed and self.ekf_on and self.active:
                    self.replan(initial=False)
            return
        ...
        for det in bboxes:
            ...
            # Ignore if too close to a known target position (don't block targets)
            if any(math.hypot(ox - tx, oy - ty) <= 0.30 for tx, ty in known_targets):
                continue

            # Ignore duplicates amongst known and discovered obstacles using 30 cm gate
            ...
            # Rate limit adds
            if not hasattr(self, "last_obstacle_add_time"):
        ...
        # Merge at most twice per second, or immediately if we added something
        now2 = time.time()
        if new_added or (now2 - self._last_merge_time) >= self.merge_cooldown:
            changed = self._merge_discovered_obstacles()
            self._last_merge_time = now2
            if (new_added or changed) and self.ekf_on and self.active:
                self.replan(initial=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Level 3: RRT with online obstacle discovery + 30cm merge")
    ...
    goals = [[-0.8927999999999999, -0.4608], [0.5088, 0.8640], [-0.9887999999999999,0.9792 ], [1.0560, -1.0080], [-1.0272, -1.0272]]
    ...
    while running:
        ...
        operate.periodic_perception_update() # add & merge obstacles + replan if needed
        operate.draw(canvas)
