...
class AutoOperateDynamic(Operate):
    ...
    def __init__(self, args,
        ...
        self.active = True
        # Manual driving mode: when True the robot will be driven with keyboard arrows
        # (UP/DOWN = forward/back, LEFT/RIGHT = rotate). Autonomous planning is bypassed
        # but SLAM, perception and logging continue.
        ...
        self._initial_spin_duration = 0  # seconds (user request)
    ...
    def _perform_calib_scan_step(self):
        ...
        if len(taglist) > 0:
            ...
            # stop rotation
            self.command['motion'] = [0, 0]
            self.notification = "ArUco seen — calibration finished, rewhsuming path"
            return
    ...
    def auto_nav_step(self):
        ...
        # Manual driving short-circuit: if manual_drive is enabled, read keyboard arrows
        # to set motion commands and skip all autonomous behaviors. SLAM, perception and
        # logging will continue in the main loop (update_slam, periodic_perception_update, etc.).
    def auto_nav_step(self):
    # --- Manual driving takes absolute priority, even if SLAM is off ---
        if getattr(self, 'manual_drive', True):
            try:
                # Make sure pygame updates keyboard state if your update loop
                # didn't already pump events
                try:
                    pygame.event.pump()
                except Exception:
                    pass
                ...
                ang = 0
                if keys[pygame.K_UP]:
                    lin = int(self.fwd_cmd)
                elif keys[pygame.K_DOWN]:
                    lin = -int(self.fwd_cmd)
                if keys[pygame.K_LEFT]:
                    ang = int(self.turn_cmd)
                elif keys[pygame.K_RIGHT]:
                    ang = -int(self.turn_cmd)
                #print(lin,ang)
                self.command['motion'] = [lin, ang]
                # Helpful HUD
                try:
                    fwd_s = 'F' if lin > 0 else ('B' if lin < 0 else '-')
                    rot_s = 'L' if ang > 0 else ('R' if ang < 0 else '-')
                    self.notification = f'Manual drive: {fwd_s} {rot_s}'
                except Exception:
                    self.notification = 'Manual drive'
            except Exception:
                self.command['motion'] = [lin, ang]
            return
